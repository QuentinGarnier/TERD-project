            ...
            BuyItemButton(AbstractCollectableItem ai) {
                ...
                ActionListener al = e -> {
                    ...
                    if (Player.getInventory().size() >= Player.MAX_INVENTORY_SIZE) {
                        ...
                        JOptionPane.showMessageDialog(Merchant.getInstanceMerchant().getMarketWindow(), "pas d'espace", "test", JOptionPane.WARNING_MESSAGE, instanceMerchant.merchantIcon);
                        restoreFocus();
                        return;
                    }
                    if (ai instanceof ItemEquip) {
